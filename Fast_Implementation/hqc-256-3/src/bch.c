...
void bch_code_encode(uint8_t* em, uint8_t* m) {
  ...
  lfsr_encoder(tmp2, (uint8_t*)g, tmp1);
...
void bch_code_decode(uint8_t* m, uint8_t* em) {
  ...
  // table_alpha_ij has been precomputed by the call of the function syndrome_gen_init() in file patch2.c, by the main.
  syndrome_set* synd_set = syndrome_init2(); 
  syndrome_gen2(synd_set, table_alpha_ij, em); 
  ...
  syndrome_gen(synd_set, tables, em); 
	...
	/*puts("syndrome_gen");
	for(int j=0;j<PARAM_DELTA<<1;j++) printf("%4x ",synd_set->tab[j]);
	puts("");*/
  #endif
...
cyclotomic_sets* cyclotomic_init() {
  ...
  c_tab->nb_c = 0;
  c_tab->tab = calloc(PARAM_GF_MUL_ORDER + 1,sizeof(int16_t));
  return c_tab;
}
