...
void hqc_pke_keygen(unsigned char* pk, unsigned char* sk) {
  ...
  gf2x_Mult((uint64_t *) s,(uint64_t *) h,(uint64_t *) y_c);
  //fastConvolutionMult((uint64_t *) h, y,(uint64_t *) s, PARAM_OMEGA);
...
void hqc_pke_encrypt(uint8_t* u, uint8_t* v, uint8_t* m, unsigned char* theta, const unsigned char* pk) {
  ...
  // Compute u = r1 + r2.h
	...
	initFWVector((uint64_t *) r2_c, r2, PARAM_OMEGA_R );
	gf2x_Mult((uint64_t *) tmp2,(uint64_t *) h,(uint64_t *) r2_c);
	//fastConvolutionMult((uint64_t *) h, r2,(uint64_t *) tmp2, PARAM_OMEGA_R);
  ...
  // Compute v = m.G + s.r2 + e
	gf2x_Mult((uint64_t *) tmp2,(uint64_t *) s,(uint64_t *) r2_c);
  //fastConvolutionMult((uint64_t *) s, r2,(uint64_t *) tmp2, PARAM_OMEGA_R);
...
void hqc_pke_decrypt(uint8_t* m, uint8_t* u, uint8_t* v, const unsigned char* sk) {
  ...
  // Compute v - u.y
	...
	uint8_t y_c [SIZE8] = {0};
	initFWVector((uint64_t *) y_c, y, PARAM_OMEGA );
	gf2x_Mult((uint64_t *) tmp2,(uint64_t *) tmp1,(uint64_t *) y_c);
	//fastConvolutionMult((uint64_t *) tmp1, y,(uint64_t *) tmp2, PARAM_OMEGA);
